...
var CookieCruncher = {
  ...
  bestDeals: function(draw=false) {
    ...
    var cookiesPs = Game.cookiesPs,
        autoClick = CookieCruncher.clickers.autoClick.settings;
    if (autoClick.on) {
      cookiesPs += (autoClick.count * autoClick.perSecond) * Game.computedMouseCps;
    }
    CookieCruncher.buildings.forEach(building => {
      var time = (building.price-Game.cookies)/cookiesPs;
    ...
    output += `<div style="font-weight:bold">per second: <span class="price">${Beautify(cookiesPs)}</span>
                <br>per minute: <span class="price">${Beautify(cookiesPs*60)}</span>
                <br>per 10 minutes: <span class="price">${Beautify(cookiesPs*600)}</span>
                <br>per 30 minutes: <span class="price">${Beautify(cookiesPs*1800)}</span>
                <br>per hour: <span class="price">${Beautify(cookiesPs*3600)}</span>
                <br>per day: <span class="price">${Beautify(cookiesPs*86400)}</span>
                </div>`
